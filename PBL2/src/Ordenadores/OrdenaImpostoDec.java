...
import Util.Swap;
import Ordenadores.comparadores.ComparaImposto;
import Util.MyLinkedList;
import Model.Produto;

public class OrdenaImpostoDec {
    private ComparaImposto comparator;
    
    /**
     * Construtor da classe OrdenaImpostoDec. Instancia um objeto da classe ComparaImposto
     * que será usado no método de ordenação.
     */
    public OrdenaImpostoDec()
    { comparator = new ComparaImposto();   }
    
    /**
     * Método de ordenação recursiva decrescente baseada em 
     ...
     * @see MyLinkedList 
     */
    public MyLinkedList quickSort(MyLinkedList lista, int inicio, int fim)  //ORDENA DECRESCENTEMENTE
    {
        if (inicio < fim)
    ...
    {   Swap swap = new Swap(lista);
        Object pivo = (Produto) lista.get(inicio).getObj();
        int begin = inicio + 1, end = fim;
        
            while (begin <= end)
            {                   
                if( comparator.compare(lista.get(begin).getObj() , pivo) >= 0  )           //DECRESCENTE
                {  begin++; }                                        
