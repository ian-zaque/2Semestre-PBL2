...
public class OrdenaPrioriCres {
    private ComparaPriori comparator;
    
    /**
     * Construtor da classe OrdenaPrioriCres. Instancia um objeto da classe ComparaPriori
     * que será usado no método de ordenação.
     */ 
    public OrdenaPrioriCres()
    ...
    {   Swap swap = new Swap(lista);
        ...
        int begin = inicio + 1, end = fim;
            ...
            while (begin <= end)
            {                   
                if( comparator.compare((ItemFragil)lista.get(begin).getObj() , pivo) <= 0  )           //CRESCENTE
                {  begin++; }                                        
                
                else if( comparator.compare((ItemFragil)lista.get(end).getObj() , pivo) >= 0  )
                {  end--; } 
