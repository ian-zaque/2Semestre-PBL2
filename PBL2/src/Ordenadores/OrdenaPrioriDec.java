...
import Util.Swap;
import Ordenadores.comparadores.ComparaPriori;
import Model.Produto;
import Util.MyLinkedList;

public class OrdenaPrioriDec {
    private ComparaPriori comparator;
    
    /**
     * Construtor da classe OrdenaPrioriDec. Instancia um objeto da classe ComparaPriori
     * que será usado no método de ordenação.
     */
    public OrdenaPrioriDec()
    { comparator = new ComparaPriori();   }
    ...
    * lista e chama o método quickSort, dividindo o lado antes do índice (lado esquerdo),
    ...
    {   Swap swap = new Swap(lista);
        ...
        int begin = inicio + 1, end = fim;
            ...
            while (begin <= end)
            {                   
                if( comparator.compare((Produto)lista.get(begin).getObj() , pivo) >= 0  )           //DECRESCENTE
                {  begin++; }                                        
